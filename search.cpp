  ...
  int alpha, int beta, int depth)
{
  ...
  if (!TC.active() || ss->ply >= MAX_PLY || position_is_draw(pos, 1, isPV))
  {
...
{
  log_line("%s", TC.printThink().c_str());

  stats_start();
  auto move = search_do(pos);
  stats_print();
